...
<tr><td>/update</td><td>POST</td><td>{x: int,<br> y:int,<br> action_type:string}</td><td>if action_type == 'flag':<br>return game.toggle_flag(gid, cell)<br>else:<br>return game.open(gid, cell)</td>
</tr>
</table>
